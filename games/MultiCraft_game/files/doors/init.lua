...
local f = io.open(multicraft.get_modpath("doors")..'/init.lua', "r")
...
local function is_right(pos, clicker)
    local r1 = multicraft.get_node({x=pos.x+1, y=pos.y, z=pos.z})
    local r2 = multicraft.get_node({x=pos.x, y=pos.y, z=pos.z+1})
...
function doors:register_door(name, def)
    ...
    multicraft.register_craftitem(name, {
        ...
        on_place = function(itemstack, placer, pointed_thing)
            ...
            local pn = placer:get_player_name()
                if multicraft.is_protected(pointed_thing.above, pn) and multicraft.is_protected(pointed_thing.under, pn) then
                    return itemstack
                end
                    local ptu = pointed_thing.under
                    local nu = multicraft.get_node(ptu)
                    if multicraft.registered_nodes[nu.name].on_rightclick then
                        return multicraft.registered_nodes[nu.name].on_rightclick(ptu, nu, placer, itemstack)
                    end
                    ...
                    pt2.y = pt2.y+1
                    if
                        not multicraft.registered_nodes[multicraft.get_node(pt).name].buildable_to or
                        not multicraft.registered_nodes[multicraft.get_node(pt2).name].buildable_to or
                        not placer or
                    ...
                    local p2 = multicraft.dir_to_facedir(placer:get_look_dir())
                    ...
                    if not string.find(multicraft.get_node(pt3).name, name.."_b_") then
                        multicraft.set_node(pt, {name=name.."_b_1", param2=p2})
                        multicraft.set_node(pt2, {name=name.."_t_1", param2=p2})
                    else
                        multicraft.set_node(pt, {name=name.."_b_2", param2=p2})
                        multicraft.set_node(pt2, {name=name.."_t_2", param2=p2})
                    end

                    if def.only_placer_can_open then
                        local pn = placer:get_player_name()
                        local meta = multicraft.get_meta(pt)
                        meta:set_string("doors_owner", "")
                        --meta:set_string("infotext", "Owned by "..pn)
                        meta = multicraft.get_meta(pt2)
                        meta:set_string("doors_owner", "")
                        --meta:set_string("infotext", "Owned by "..pn)
                    end
    ...
    local function after_dig_node(pos, name, digger)
        ...
        if node.name == name then
            multicraft.node_dig(pos, node, digger)
        end
    end
        ...
        on_rightclick = function(pos, node, clicker)
            ...
            on_rightclick(pos, 1, name.."_t_1", name.."_b_2", name.."_t_2", {1,2,3,0})
                if is_right(pos, clicker) then
                    multicraft.sound_play("door_close", {pos = pos, gain = 0.3, max_hear_distance = 10})
                else
                    multicraft.sound_play("door_open", {pos = pos, gain = 0.3, max_hear_distance = 10})
                end
            end
        end,
    ...
    multicraft.register_node(name.."_t_1", {
    ...
    multicraft.register_node(name.."_t_2", {
...
multicraft.register_alias("doors:door_wood_a_c", "doors:door_wood_t_1")
multicraft.register_alias("doors:door_wood_a_o", "doors:door_wood_t_1")
multicraft.register_alias("doors:door_wood_b_c", "doors:door_wood_b_1")
multicraft.register_alias("doors:door_wood_b_o", "doors:door_wood_b_1")
...
local function punch(pos)
    meta = multicraft.get_meta(pos)
    state = meta:get_int("state")
    me = multicraft.get_node(pos)
    local tmp_node
    local tmp_node2
...
multicraft.register_node("doors:trapdoor", {
...
multicraft.register_node("doors:trapdoor_open", {
    drawtype = "nodebox",
...
multicraft.register_node("doors:iron_trapdoor", {
    description = "Trapdoor",
    drawtype = "nodebox",
...
multicraft.register_node("doors:iron_trapdoor_open", {
    drawtype = "nodebox",
